
      
        
        
<div class="container-fluid" style="min-height: 200px;">
    <div class="row">
        
        <div class="row text-center" id="connection-case">
           
            
            
            <div class="col-md-4 col-sm-4 col-xs-12">
                <div class="question-block hvr hvr-bounce-in" id="report"> 
                    <a href="https://nebesa.me/main/a_vie/content/adminpages/report.php#">
                        <div class="col-md-12"><span class="flaticon flaticon-data-analytics"></span></div>
                        <div class="col-md-12 text-lowercase"><h5>Конструктор <br> запросов <br>ЖКХ</h5></div>                    
                    </a>
                </div>
            </div>
            <div class="col-md-4 col-sm-4 col-xs-6">
                <div class="question-block hvr hvr-bounce-in" id="action">                   
                    <div class="col-md-12"><span class="flaticon flaticon-fair15"></span></div>
                    <div class="col-md-12 text-lowercase"><h5>Вопросы <br> по заказу <br> мероприятий</h5></div>                    
                </div>
            </div>
            <div class="col-md-4 col-sm-4 col-xs-6">
                
                <div class="question-block hvr hvr-bounce-in" id="sponsors">
                     <a href="https://nebesa.me/main/a_vie/content/adminpages/amazon.php#">
                    <div class="col-md-12"><span class="flaticon flaticon-data-analytics"></span></div>
                    <div class="col-md-12 text-lowercase"><h5>Анализ <br> продукции </h5></div>
                     </a>
                </div>
            </div>           
        </div>
        <div class="row" id="report-body" style="display: none;">
             <div id="output" style="margin: 30px;"></div>
        </div>
       
    </div>
    </div>

<script type="application/javascript">
    
     $(document).ready(function () {
         
         var theme;
         /*
         $('.question-block').on('click', function(event) {
                                    event.preventDefault();
                                    
                  theme = $(this).attr('id');
                //  console.log(theme);
                  $('#connection-case').animate({'marginLeft':'-120%'},{duration:400,queue:false});   
                  $('#connection-case').fadeOut(400);
                //тип ресурса
                if (theme == 'report') {
                    
                  $('#header-message').text('Конструктор отчетов');
                  $('#report-body').fadeIn(600);
                  $('#report-body').animate({'marginLeft':'0%'},{duration:400,queue:false});                  
                  $('.modal-lg').animate({width: '95%'},900);
                  $('#success').fadeIn(600);
                  $('#back').fadeIn(600);
                }
            });
            
             $('#back').on('click', function(event) {                                
                                    event.preventDefault();
                  $('#success').fadeOut(600);
                  $('#back').fadeOut(600);                  
                  $('#report-body').animate({'marginLeft':'-120%'},{duration:400,queue:false});   
                  $('#report-body').fadeOut(400);
                  $('.modal-lg').animate({width: '950px'},900);
                  $('#header-message').text('Выберите тему Ваших коммуникаций');
                  $('#connection-case').fadeIn(600);
                  $('#connection-case').animate({'marginLeft':'0%'},{duration:400,queue:false});     
                 
       
       
            });
            */
  
            
     });
     
     
     
     
     
     
     
     
     
     
</script>